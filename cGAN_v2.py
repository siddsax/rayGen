...
parser.add_argument('--dataPath', type=str, default="dataset", help='dataset path')
...
class Generator(nn.Module):
    def __init__(self):
        ...
        a = block(opt.latent_dim+opt.n_classes, 128, normalize=False)\
            + block(128, 256) \
            + block(256, 512) \
            + block(512, 1024) \
...
from utils import rayleighData
...
dataloader = torch.utils.data.DataLoader(rayleighData(opt.dataPath, 
                   transform=transforms.Compose([
                         transforms.Resize((opt.img_size, opt.img_size)),
                         transforms.ToTensor(),
                         transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
...
def sample_image(n_row, batches_done):
    ...
    save_image(gen_imgs.data, 'images/n_v2%d.png' % batches_done, nrow=n_row, normalize=True)
